...
/*inline void processor_t::update_tracked_insn_count(const insn_t & insn)
{
  try {
    tracked_insn_count.at(get_disassembler()->disassemble(insn).c_str())++;      // vector::at throws an out-of-range
  }
  catch (const std::out_of_range& oor) {
 	fprintf(stderr, "Instruction is not found/tracked\n");
  }
...
static reg_t execute_insn(processor_t* p, reg_t pc, insn_fetch_t fetch)
{
  ...
  if(p->get_track_insn())
     p->update_tracked_insn_count(fetch.insn);
