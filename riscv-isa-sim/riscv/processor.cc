...
processor_t::~processor_t()
{
  ...
  if (track_insn)
  {
    fprintf(stderr, "Tracked Instruction Size:%zu\n", tracked_insn_count.size());
    for (auto it : tracked_insn_count)
...
void processor_t::set_track_insn( bool value)
{
  track_insn = value;
}

void processor_t::set_tracked_insns(const std::vector<std::string> & value ){
   for(std::string insn : value){
      tracked_insn_count[insn]; //initialize to zero
   }
}
...
void processor_t::update_tracked_insn_count(const insn_t & insn)
{
  std::string insn_name = (disassembler->lookup(insn))->get_name();
  tracked_insn_count[insn_name]++;      // vector::at throws an out-of-range
 
  //tracked_insn_count[p->disassembler->disassemble(insn).c_str()]++;
}
