...
sim_t::sim_t(const char* isa, size_t nprocs, bool halted, reg_t start_pc,
             ...
             std::vector<int> const hartids, std::vector<std::string> const 
	     tracked_insn, unsigned progsize, unsigned max_bus_master_bits, bool 
    ...
    debug_module(this, progsize, max_bus_master_bits, require_authentication)
{
  ...
  if(tracked_insn.size() == 0) {
    for (size_t i = 0; i < procs.size(); i++) {
      procs[i]->set_tracked_insns(tracked_insn);
    }
  }
...
void sim_t::set_track_insn( bool value)
{
  track_insn = value;
}
...
void sim_t::set_procs_track_insn(bool value)
{
  for (size_t i = 0; i < procs.size(); i++) {
    procs[i]->set_track_insn(value);
  }
}
