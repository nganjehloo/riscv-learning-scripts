...
static void help()
{
  ...
  fprintf(stderr, "  -t<addi, ld,..>                    Count the amount of times specified instructions were executed\n");
...
int main(int argc, char** argv)
{
  ...
  std::vector<std::string> tracked_insn;

  auto const tracked_insn_parser = [&](const char *s) {
    std::string str(s);
   ...
   while ((pos = str.find(delimiter)) != std::string::npos) {
      ...
      printf( "Tracking %s", token.c_str());
      tracked_insn.push_back(token);
  ...
  parser.option('t', 0, 1, tracked_insn_parser);
  ...
  sim_t s(isa, nprocs, halted, start_pc, mems, htif_args, std::move(hartids),
      std::move(tracked_insn), progsize, max_bus_master_bits, require_authentication);
  ...
  s.set_debug(debug);
  s.set_procs_track_insn(track_insn);
  s.set_log(log);
